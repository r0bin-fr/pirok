/*
  ...
  * Adapted by Matthieu Hameau (2015) for flow sensor
  * 
...
typedef struct {
	long count;                  /* Number of interrupts received */
...
/* This function is called when the 'flow' kernel object is read */
//static ssize_t range_show(
static ssize_t flow_show(
...
static struct kobj_attribute rangeAttribute = __ATTR_RO(flow);
...
static int __init moduleInit( void )
{
	...
	s_kernelObject = kobject_create_and_add( "flow", kernel_kobj );
	...
	/* Request an IRQ for the echo GPIO pin, so that we can measure the rising
	 * and falling edge of the pulse from the flow.
	 */
    myIRQ = gpio_to_irq( echo );
    if ( request_irq(
		myIRQ,
		gpioInterruptHandler,
		IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_SHARED,
